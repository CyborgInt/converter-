"""
CLI обёртка для конвертера.
Примеры:
  python -m svc --from csv --to json --input data.csv --output data.json
  python -m svc --serve  # запускает сервер (те же операции через HTTP)
"""
...
def parse_args(argv=None):
    p = argparse.ArgumentParser(prog="svc", description="CSV <-> JSON <-> YAML converter")
    ...
    p.add_argument("--pretty", action="store_true", help="Pretty JSON output (для json).")
...
def convert_text(src_fmt: str, dst_fmt: str, text: str, delimiter: str = ",", pretty: bool = True) -> str:
    ...
    raise ValueError(f"Unsupported conversion: {src_fmt} -> {dst_fmt}")


def main(argv=None):
    args = parse_args(argv)
    if args.serve:
        # Запустим сервер (делегируем uvicorn запуск в модуле api)
        from .api import run_from_cli
        run_from_cli()
        return

    if not args.src_format or not args.dst_format:
        print("Error: --from and --to are required for CLI conversion (unless --serve).", file=sys.stderr)
        sys.exit(2)
